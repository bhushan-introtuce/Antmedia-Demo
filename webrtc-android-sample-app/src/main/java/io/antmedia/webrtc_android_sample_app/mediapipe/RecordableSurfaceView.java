...
public class RecordableSurfaceView extends SurfaceView {
    ...
    public void initRecorder(File saveToFile, int displayWidth, int displayHeight,
        ...
        try {
            ...
            Log.d("Recording_Debug>>", "Init Done");

        } catch (Exception e) {
            Log.d("Recording_Debug>>", e.toString());
            Toast.makeText(getContext(), "Failed Recotrder", Toast.LENGTH_SHORT).show();
            mMediaRecorder = null;
            // releaseRecorder();
        }
     ...
     */
    public interface FrameCaptureListner {
        void onBitmap(Bitmap bitmap);
    }

    protected FrameCaptureListner frameCaptureListner;

    public FrameCaptureListner getFrameCaptureListner() {
        return frameCaptureListner;
    }

    public void setFrameCaptureListner(FrameCaptureListner frameCaptureListner) {
        this.frameCaptureListner = frameCaptureListner;
    }

    protected boolean isCaptureRequest = false;
    ...
    private class ARRenderThread extends Thread implements SurfaceHolder.Callback2 {
        ...
        public void surfaceCreated(SurfaceHolder surfaceHolder) {
            ...
            if (mRendererCallbacksWeakReference != null
                ...
                mRendererCallbacksWeakReference.get().onSurfaceCreated(surfaceHolder);
    ...
    protected Bitmap createBitmapFromGLSurface(int x, int y, int w, int h)
